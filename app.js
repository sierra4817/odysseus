...
let sentencesList = []; // Flat list of sentences in the current chapter
let isPlaying = false;
let currentUtterance = null;
let selectedVoice = null;
let playbackRate = 1.0;
let playerMode = 'audio'; // 'audio' (Professional MP3) or 'tts' (Synthesized Speech)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initTTS();
    setupEventListeners();
    setPlayerMode('audio'); // Default mode
});
...
// ==========================================================================
function initTheme() {
    const storedTheme = localStorage.getItem('tts_reader_theme');
...
function setupEventListeners() {
    ...
    themeToggle.addEventListener('click', () => {
        ...
        localStorage.setItem('tts_reader_theme', isLight ? 'light' : 'dark');
    });
    ...
    // Mode toggles
    modeAudioBtn.addEventListener('click', () => setPlayerMode('audio'));
    modeTtsBtn.addEventListener('click', () => setPlayerMode('tts'));
...
function updateProgressBar() {
    if (playerMode === 'audio') {
        ...
        progressPercent.textContent = `Oración ${current} / ${total}`;
...
function handleProgressScrub(e) {
    ...
    if (playerMode === 'audio') {
        ...
        if (totalSentences > 0) {
            const targetSentenceIdx = Math.floor(clickPercent * totalSentences);
            speakSentence(targetSentenceIdx);
        }
    }
}
...
function setPlayerMode(mode) {
    ...
    playerMode = mode;
    localStorage.setItem('tts_reader_player_mode', mode);
    ...
    if (mode === 'audio') {
        ...
        btnPrev.title = "Retroceder 10s";
        btnNext.title = "Adelantar 10s";
    } else {
        modeTtsBtn.classList.add('active');
        modeAudioBtn.classList.remove('active');
        voiceControlsContainer.classList.remove('hidden');
        
        btnPrev.title = "Oración anterior";
        btnNext.title = "Siguiente oración";
    }
...
// ==========================================================================
// PLAYBACK CONTROLLER (Real Audio + TTS Synthesis)
// ==========================================================================
...
// Keep a simple global reference to prevent voice cutting issues on Chrome
...
// Handle automatic play of the next unit when audio finishes
...
function populateVoices() {
    ...
    if (spanishVoices.length === 0) {
        ...
        spanishVoices.forEach(voice => {
            ...
            if (voice.name.includes('Google') || voice.name.includes('Helena') || voice.name.includes('Sabina')) {
...
function speakSentence(index) {
    ...
    activeSpeechUtterance = currentUtterance;
